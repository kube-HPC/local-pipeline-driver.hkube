...
const NodeState = require("./NodeStates");
...
class StateManager extends EventEmitter {
  ...
  async runAlgorithm(job) {
    ...
    if (job.data.tasks.length === 1) {
      const input = { ...job.data, input: job.data.tasks[0].input, status: NodeState.ACTIVE, taskId: job.data.tasks[0].taskId, startTime: Date.now() };
      ...
      for (const task of job.data.tasks) {
        const jobBuild = { ...job.data, input: task.input, status: NodeState.ACTIVE, taskId: task.taskId, startTime: Date.now() };
  ...
  handleAlgorithmResult(job, type) {
    ...
    if (type === recivedType.ALGORTIHM_FINISHED_SUCCESS) {
      this.emit(Events.TASKS.SUCCEED, { ...job.data, status: NodeState.SUCCEED, result: job.result, taskId: job.data.taskId, endTime: Date.now() });
    } else {
      this.emit(Events.TASKS.FAILED, { ...job.data, status: NodeState.FAILED, taskId: job.data.taskId, endTime: Date.now() });
    }
  }
