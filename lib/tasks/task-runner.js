...
const { parser, consts } = require('../parser/index');
const StateManager = require('../state/state-manager'); // to wrap
...
const { PipelineReprocess } = require('../errors');
const { sendType } = require('../consts/ws-messages');
...
class TaskRunner extends EventEmitter {
    ...
    init() {
        ...
        this._stateManager.on(Events.JOBS.START, (data) => {
            this.start(data.job);
        });
    ...
    async _startPipeline(job) {
        this._job = job;
        this._jobId = this._createJobId(job.name);
        this._jobStatus = DriverStates.ACTIVE;
        log.info(`pipeline started ${this._jobId}`.yellow, {
            component,
            jobId: this._jobId
        });

        const jobStatus = await this._stateManager.getJobStatus({
            jobId: this._jobId
        });
        ...
        // const pipeline = await this._stateManager.getExecution({
        //   jobId: this._jobId
        // });
        // if (!pipeline) {
        ...
        this.pipeline = this._job;
        // const res = parser.checkFlowInput({ nodeInput: this.pipeline.nodes[0].input[0], ...this.pipeline });
        //   const res = parser.parse({ nodeInput: this.pipeline.nodes[0].input, flowInput: this.pipeline.flowInput });
        this._nodes = new NodesMap(this.pipeline, { parser: { parser, consts } });
        ...
        await this._progress.info({
            jobId: this._jobId,
            pipeline: this.pipeline.name,
            status: DriverStates.ACTIVE
        });
    ...
    async _stopPipeline(err, reason) {
        ...
        if (err) {
            ...
            log.error(`pipeline ${status}. ${errorMsg}`, {
                component,
                jobId: this._jobId,
            ...
            log.info(`pipeline ${status} ${this._jobId}. ${reason}`, {
                component,
                jobId: this._jobId,
                pipelineName: this.pipeline.name
            });
            ...
            log.info(`pipeline ${status} ${this._jobId}`.yellow, {
                component,
                jobId: this._jobId,
                pipelineName: this.pipeline.name
            });
            data = this._nodes.pipelineResults();
            this._stateManager.emit(sendType.PIPELINE_FINISHED, data);
        }
        this._jobStatus = status;
        this._driverStatus = DriverStates.READY;
        const resultError = await this._stateManager.setJobResults({
            jobId: this._jobId,
            startTime: this.pipeline.startTime,
            pipeline: this.pipeline.name,
            data,
            reason,
            error: errorMsg,
    ...
    _runEntryNodes() {
        let entryNodes;
        if (this._pipeline.kind === 'stream') {
            entryNodes = this._nodes._graph.nodes();
        }
        else {
            entryNodes = this._nodes.findEntryNodes();
        }
    ...
    //   const watchState = await this._stateManager.watchJobState({
    //     jobId: this._jobId
    //   });
    ...
    async _unWatchJob() {
        try {
            await Promise.all([this._stateManager.unWatchJobState({ jobId: this._jobId }), this._stateManager.unWatchTasks({ jobId: this._jobId })]);
        }
        catch (e) {
            log.error(e.message, { component, jobId: this._jobId }, e);
        }
    ...
    async _deletePipeline() {
        try {
            await Promise.all([this._stateManager.deleteTasksList({ jobId: this._jobId })]);
        }
        catch (e) {
            log.error(e.message, { component, jobId: this._jobId }, e);
        }
    ...
    async _progressError({ status, error }) {
        if (this._progress) {
            ...
            await this._stateManager.setJobStatus({
                jobId: this._jobId,
                pipeline: this.pipeline.name,
                status,
                error
    ...
    async _runNode(nodeName, parentOutput, index) {
        try {
            log.info(`node ${nodeName} is ready to run`.yellow, { component });
            ...
            //  dataExtractor.extract(result, result.storage);
    ...
    _setTaskState(task) {
        ...
        if (error) {
            log.error(`task ${status} ${taskId}. error: ${error}`, {
                component,
                jobId: this._jobId,
                pipelineName: this.pipeline.name,
                taskId
    ...
    _createJob(options, batch) {
        ...
        return this._stateManager.runAlgorithm(jobOptions);
    }
}
